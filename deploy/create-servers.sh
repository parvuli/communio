...
if ! test -f "${SCRIPT_DIR}/persistent.${ENV}.tfvars"; then
  echo "File persistent.${ENV}.tfvars not found - run create-zone.sh before running this script."
  exit 1
fi
...
terraform -chdir=deploy apply -auto-approve -var="env=${ENV}" -var="num_validator_instances=$NUM_VALIDATORS" -var="num_seed_instances=${NUM_SEEDS}" -var="create_explorer=true" -var-file="persistent.${ENV}.tfvars"
